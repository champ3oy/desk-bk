...
import { EmailIntegrationStatus, EmailProvider } from './entities/email-integration.entity';
...
export class OutlookPollingService {
  ...
  async pollEmails() {
    ...
    try {
      ...
      const outlookIntegrations = integrations.filter(i => i.provider === EmailProvider.OUTLOOK);
      
      this.logger.debug(`Found ${outlookIntegrations.length} active Outlook integrations to poll`);

      for (const integration of outlookIntegrations) {
        try {
          this.logger.debug(`Polling Outlook integration for email: ${integration.email}`);
  ...
  async syncMessagesManually(integration: any, days: number) {
      ...
      this.logger.log(`Starting manual sync for ${integration.email} looking back ${days} days (>= ${queryDate})...`);
  ...
  private async fetchMessages(integration: any, dateIsoString: string) {
    ...
    while (hasNext && pageCount < maxPages) {
        ...
        this.logger.debug(`Found ${messages.length} messages for ${integration.email} in page ${pageCount + 1}`);
        ...
        for (const msg of messagesToProcess) {
            try {
                const mappedMessage = this.mapOutlookMessageToDto(msg, integration.email);
                ...
                if (result.success) {
                    this.logger.debug(`Ingested Outlook message ${mappedMessage.messageId}`);
                }
                // We ignore duplicates gracefully usually
            } catch (err) {
                this.logger.error(`Error processing Outlook message ${msg.id}: ${err.message}`);
            }
        }
  ...
  private mapOutlookMessageToDto(msg: any, connectedEmail: string): IncomingMessageDto {
      ...
      if (!fromEmail && msg.internetMessageHeaders) {
          const findHeader = (name: string) => msg.internetMessageHeaders.find((h: any) => h.name.toLowerCase() === name.toLowerCase())?.value;
      ...
      if (!fromEmail) {
          this.logger.error(`Message ${msg.id} has no sender email. Subject: ${msg.subject}`);
          ...
          throw new Error(`Cannot process message ${msg.id}: no sender email found`);
      }
      
      this.logger.debug(`Extracted sender: ${fromEmail} (${fromName}) for message ${msg.id}`);
      ...
      if (msg.body) {
          ...
          // Check if content is HTML and convert to text
          if (msg.body.contentType === 'html' || rawContent.trim().startsWith('<')) {
      ...
      this.logger.debug(`Returning DTO: senderEmail=${dto.senderEmail}, recipientEmail=${dto.recipientEmail}, subject=${dto.subject}`);
