...
export class EmailIntegrationController {
  ...
  async manualSync(@Body() dto: ManualSyncDto, @Request() req) {
    const integration = await this.emailIntegrationService.findByEmail(
      dto.email,
    );
    if (!integration) {
    ...
    if (integration.organizationId.toString() !== req.user.organizationId) {
      throw new HttpException(
        'Unauthorized access to this integration',
    ...
    if (integration.provider === EmailProvider.OUTLOOK) {
      await this.outlookPollingService.syncMessagesManually(
        integration,
        dto.days,
      );
    } else {
      await this.gmailPollingService.syncMessagesManually(
        integration,
        dto.days,
      );
    }
  ...
  async handleGoogleCallback(@Body() dto: GoogleCallbackDto, @Request() req) {
  ...
  async handleOutlookCallback(@Body() dto: OutlookCallbackDto, @Request() req) {
  ...
  @Patch(':id/toggle')
  @Roles(UserRole.ADMIN)
  @ApiOperation({ summary: 'Toggle email integration active status' })
  async toggleStatus(@Param('id') id: string, @Request() req) {
    return this.emailIntegrationService.toggleStatus(
      id,
      req.user.organizationId,
    );
  }
